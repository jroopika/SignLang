...
actions = np.array(['hello','yes','no', 'thankyou'])
...
# Create directories for each action and sequence in the dataset
    ...
    for action, sequence, frame in product(actions, range(sequences), range(frames)):
        ...
        if frame == 0:
            while True:
                ...
                cv2.putText(
                    image,
                    'Recording data for the "{}". Sequence number {}.'.format(action, sequence),
                    (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
                cv2.putText(
                    image, 'Pause.', (20, 400),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
                cv2.putText(
                    image, 'Press "Space" when you are ready.', (20, 450),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
...
# Release the camera and close any remaining windows (OUTSIDE the 'with' block)
cap.release()
